...
```
                    指尖
                 ╭───●───╮
                 │ 8 12 16 │  ← 食指/中指/无名指/小指 指尖
                 │ 7 11 15 │  ← PIP 关节（近端指间关节）
                 │ 6 10 14 │  ← DIP 关节（远端指间关节）
                 │       20│  ← 小指 MCP 关节
     拇指 → ●4  ●5 ●9 ●13 ●17
             ●3  ●2       ●18  ← 手掌区域
                ●1        ●19  ← 手掌基部
...
|------|----------|------|------|
| 手腕 | 0 | 1 | 手腕根部中心点 |
| 拇指 | 1~4 | 4 | CMC → MCP → IP → 指尖 |
| 食指 | 5~8 | 4 | MCP → PIP → DIP → 指尖 |
| 中指 | 9~12 | 4 | MCP → PIP → DIP → 指尖 |
| 无名指 | 13~16 | 4 | MCP → PIP → DIP → 指尖 |
| 小指 | 17~20 | 4 | MCP → PIP → DIP → 指尖 |

> 每个关键点输出 (x, y, z) 三维坐标，z 坐标表示相对于手腕的深度，可用于手势识别和 3D 手部建模。
